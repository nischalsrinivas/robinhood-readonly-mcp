...
echo "Testing robinhood-readonly-mcp module import and server creation..."
...
if command -v robinhood-readonly-mcp &>/dev/null; then
  echo "✓ Entry point 'robinhood-readonly-mcp' found"
else
  # Entry point might not be in PATH in CI, check with pip
  pip show robinhood-readonly-mcp &>/dev/null && echo "✓ Package installed" || {
